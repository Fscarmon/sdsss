        ...
        for _ in range(num_emails):
            ...
            while email_retry_count < max_email_retries:
                try:
                    ...
                    if usernames is None:  # Handle the case where get_user_name failed
                        email_retry_count += 1
                        logger.warning(f"获取用户名失败, 邮箱 {email} 尝试次数: {email_retry_count}/{max_email_retries}")
                        continue
                    _ = usernames.pop()
                    first_name = _["name"]
                    last_name = _["surname"]
                    ...
                    with cloudflare_scrape.create_scraper() as session:
                        ...
                        try:
                            resp = session.get(url=url1, headers=session.headers)
                            ...
                            while True:
                                ...
                                logger.info("获取验证码")
                                resp = session.get(url=captcha_url.format(captcha_0), headers=dict(header2, **{"Cookie": header2["Cookie"].format(csrftoken)}))
                            ...
                            try:
                                ...
                                logger.error(f"\033[7mPOST 请求发生异常:{e},正在重新开始任务...\033[0m")
                                time.sleep(random.uniform(0.5, 1.2))
                                email_retry_count += 1
                        except requests.exceptions.RequestException as e:
                             logger.error(f"\033[7mGET 请求发生异常:{e},正在重新开始任务...\033[0m")
                             time.sleep(random.uniform(0.5, 1.2))
                             email_retry_count +=1
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.error(f"Initial requests failed: {e}")
        exit()
