    ...
    while email_retry_count < max_email_retries:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
               ...
               async with session.get(url=url1, headers=headers, ssl=False) as resp:
                   ...
                   while True:
                      ...
                      logger.info("获取验证码")
                      async with session.get(url=captcha_url.format(captcha_0), headers=dict(header2, **{"Cookie": header2["Cookie"].format(csrftoken)}), ssl=False) as resp:
                   ...
                   await asyncio.sleep(random.uniform(0.5, 1.2))
                   logger.info("请求信息")
                   async with session.post(url=url3, headers=dict(header3, **{"Cookie": header3["Cookie"].format(csrftoken)}), data=data, ssl=False) as resp:
                        logger.info(f'请求状态码: \033[1;93m{resp.status}\033[0m')
                        try:
                            content = await resp.json()
                            if resp.status == 200 and len(content.keys()) == 2:
            ...
            logger.error(f"\033[7mAiohttp ClientError: {e}, 正在重试...\033[0m")
            await asyncio.sleep(random.uniform(0.5, 1.2))
            email_retry_count += 1
            continue
        except Exception as e:
